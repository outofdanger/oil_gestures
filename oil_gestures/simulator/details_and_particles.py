...
class Valve(Detail):
    ...
    @property
    def is_open(self):
        # Совместимость с CommandDispatcher (THUMB_UP): решает open/close.
        return self._opened
...
class ParticleSystem:
    ...
    def __init__(self, plotter, position, direction=(0, 1, 0), particle_type=OIL, count=400,
        ...
        self._count = count
        self._active = False
        # На некоторых Windows/GPU контекстах шейдер точек-сфер не компилируется
        # (как SSAA) -> точки не видно. Флаг позволяет откатиться на плоские
        # точки (OIL_POINT_SPHERES=0, см. RenderProfile). Основные капли нефти
        # рисуются glyph-геометрией и от этого флага не зависят.
        ...
        if particle_type == self.OIL:
            ...
            self._actor = plotter.add_mesh(
                ...
                render_points_as_spheres=self._render_points_as_spheres,
                opacity=self._opacity,
                lighting=True,
                smooth_shading=True,
            )
