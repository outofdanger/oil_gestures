...
class Controller(QObject):
    ...
    def _on_inventory_click(self, name: str):
        ...
        if detail and hasattr(detail, "attach"):
            ...
            self.panel.set_inventory(self.model.get_inventory())
            self.panel.set_model_state(f"{self.model.get_display_name(detail)}: установлен(а)")
    ...
    def _show_help(self):
        ...
        text = QLabel("""
        🖱️ Мышь:
        • ЛКМ (клик по детали) — активировать/использовать деталь
        ...
        • Esc — сброс камеры и возврат к исходному виду
          (в т.ч. выход из зума на уровнемер/контроллер/манометр)

        ✋ Жесты:
        • 👍 Большой палец вверх — активировать выбранное
          (открыть/закрыть вентиль, снять/поставить манометр
          или заглушку, нажать кнопку/рычаг)
        • ✌ Виктори — вкл/выкл курсор-режим
        • 👉 Указательный палец — открыть меню выбранной детали
        • 👈 / 👉 Свайп влево/вправо — переключить выбор
        • 🔄 Вращение по/против часовой — приоткрыть/прикрыть
          выбранный вентиль на шаг
        • ✊ Сжать кисть — приблизиться к выбранному узлу
        • 🖐 Разжать кисть — вернуться в общий вид
        
        🖐️ Курсор-режим:
        • Пока курсор-режим включён, жесты сцены не работают — рука
          управляет только системным курсором мыши
        • 🖐 Двигайте рукой — курсор следует за ней
        • 🤏 Щипок большим + указательным пальцами — левая кнопка
          мыши: короткий щипок = клик, щипок с удержанием и
          движением руки = перетаскивание
        • 🤏 Щипок большим + средним пальцами — клик правой кнопкой
        • Если рука пропадает из кадра во время зажатия — левая
          кнопка отпускается автоматически

        ⚠️ Аварийный стоп — используйте кнопку «АВАРИЙНЫЙ СТОП»
        на панели или большую жёлтую кнопку на контроллере.
        """)
        ...
        🎛️ Контроллер — порядок работы:
         ...
         6. Режим "Понизить дебит":
            уменьшает ток, частоту и заданное давление.
         7. Большая желтая кнопка — аварийная остановка.
            При нажатии контроллер выключается,
            рычаг возвращается вверх,
            индикаторы гаснут.
         8. Для выключения контроллера можно также перевести рычаг вверх.
