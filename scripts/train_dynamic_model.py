...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--data", type=str, default="data/processed/dynamic_gestures_v1.pt")
    parser.add_argument("--output", type=str, default="assets/models/pytorch/dynamic_bilstm.pt")
    parser.add_argument("--onnx-output", type=str, default="assets/models/onnx/dynamic_bilstm.onnx")
    parser.add_argument("--report", type=str, default="data/processed/dynamic_training_report.json")
...
def export_onnx(
    model: "nn.Module", onnx_path: Path, target_len: int, feature_dim: int, device: "torch.device"
) -> tuple[bool, str]:
    try:
        onnx_path.parent.mkdir(parents=True, exist_ok=True)
        model.eval()
        dummy_input = torch.randn(1, target_len, feature_dim, device=device)
        torch.onnx.export(
            model,
            dummy_input,
            str(onnx_path),
...
def main() -> int:
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss, train_acc = train_one_epoch(model, train_loader, criterion, optimizer, device, args.grad_clip)
    ...
    if args.export_onnx:
        onnx_exported, onnx_status = export_onnx(model, onnx_path, target_len, feature_dim, device)
